...
t0 = 6.35 # ns, centre tof
T0 = 29.5 # ns, put centre here
cen = [255.75, 238] # xy centre
rotangle = 32.0*np.pi/180 # xy rotation angle (camera not aligned)
...
paths = [loadpath + "213_20231213_170156/"]
...
# errors
dfitt = 0.09 # a.u. 20% p0
dfitp = 20 # 10 points corresponds to 0.5 ns
...
for i in range(0, len(tofslice)):
    ...
    else: uli = np.searchsorted(pr, pmax+dfitt)
...
plt.suptitle("dToF: {} ps; dXY: {} pixels".format(dtof*1000, dxy))
...
mask2 = np.copy(mask1)
deltap = 1/ptrue * (abs(Pz*DPz)*dtof + abs(DPt)*K*dxy)
mask1[np.sqrt(Ptrans**2+Pz**2) < ptrue + deltap] = 1
mask2[np.sqrt(Ptrans**2+Pz**2) > ptrue - deltap] = 1
