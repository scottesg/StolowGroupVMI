...
pts = np.load("EX_CH3I_20220831_2126_T1.pt.npy")
coords = plt.imread("EX_CH3I_Axes.png")

# Params
ang = -32 # rotation angle
trange = np.array([197, 210])-0.3 # Time histogram range
nlf = 0 # nonlinear factor for time points
...
d = 180 # Radius around centre to show
r = 80 # Radius of ring
...
nbin = 74 # number of frames
trange = [2.5, 11]
smth = 0
framedelay = 80 # time between frames (ms)
trim = [0, 99.4] # show between these percentiles
...
ani.save("EX_CH3I6ns_Movie.mp4", bitrate=20000)

#%% PES

# energy = 2.78

# pesI = np.load(path+"PES/ch3iPESinv.npy")
# eVI = pesI[0]
# IrI = pesI[1]
# IrI = IrI/max(IrI)

# pesXY = np.load(path+"PES/ch3iPESsliceXY.npy")
# eVXY = pesXY[0]
# IrXY = pesXY[1]
# IrXY = IrXY/max(IrXY)

# pesXZ = np.load(path+"PES/ch3iPESsliceXZ.npy")
# eVXZ = pesXZ[0]
# IrXZ = pesXZ[1]
# IrXZ = IrXZ/max(IrXZ)

# # Fitting
# i1 = np.searchsorted(eVI, 2.6)
# i2 = np.searchsorted(eVI, 2.96)
# xsI = eVI[i1:i2]
# ys = IrI[i1:i2]
# fityI, params, pcov = fg(xsI, ys, 0, np.max(IrI), energy, 0.04, czero=True)
# wI = params[2]*2.355

# i1 = np.searchsorted(eVXY, 2.6)
# i2 = np.searchsorted(eVXY, 2.96)
# xsXY = eVXY[i1:i2]
# ys = IrXY[i1:i2]
# fityXY, params, pcov = fg(xsXY, ys, 0, np.max(IrXY), energy, 0.04, czero=True)
# wXY = params[2]*2.355

# i1 = np.searchsorted(eVXZ, 2.4)
# i2 = np.searchsorted(eVXZ, 3.2)
# xsXZ = eVXZ[i1:i2]
# ys = IrXZ[i1:i2]
...
# plt.title("Photoelectron Spectrum, CH3I 6.9 ns", fontsize=25)
# plt.xlim([0, 5])
