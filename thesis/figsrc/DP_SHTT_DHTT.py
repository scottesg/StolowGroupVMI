...
trsingle = nohits + gsr*h
secondpeak = np.hstack((gsr[40:], gsr[:40]))
trdouble = nohits + (gsr + secondpeak)*h
...
filt = [0.6, 2]
gsm = 1.7 # Gaussian filter, overrides butter filter if not 0
thresh = 0.1
...
for i in range(0, len(prs)):
    ...
    # RMS Difference
    diff = np.sqrt(np.sum((pri - gsr)**2))
    rms.append(diff)
    
    # Test deconvolution of trial single-hit trace 
    shpeaks = getpeaks(trsingle, pri*h, thresh, 500, filt, 2, t, ef, gsm=gsm, gfit=True)
    if shpeaks is None:
    ...
    dhpeaks = getpeaks(trdouble, pri*h, thresh, 500, filt, 3, t, ef, gsm=gsm, gfit=True)
    if shpeaks is None:
        print("Wrong Number of Peaks! [{}]".format(i))
        dhwd.append(0)
        dhpos.append(0)
...
tstart = 120
twidth = 30
ax2.plot(t-tstart, shttnorm, lw=4, color='blue', label="Test Trace")
ax2.plot(t-tstart, deconorm, lw=2, color='black', ls="--", label="Deconvolution")
ax2.set_title("Single-Hit Test Trace", fontsize=FSt)
...
ax4.add_patch(circ2)
ax4.set_title("Time Difference (ns)", fontsize=FSt)
...
ax5.add_patch(circ3)
ax5.set_title("Amplitude Ratio", fontsize=FSt)
...
ax2.add_patch(circ4)
ax2.set_title("Time Difference, t2-t1 (ns)", fontsize=FSt)
...
ax3.add_patch(circ5)
ax3.set_title("Amplitude Ratio, A2/A1", fontsize=FSt)
